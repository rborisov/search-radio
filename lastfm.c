...
static void process_object(json_value* value, int depth)
{
    ...
    for (x = 0; x < length; x++) {
        ...
        if (strcmp(value->u.object.values[x].name, "id") == 0 && depth == 1) {
            ...
            dirstreams.current = append_stream();
            if (dirstreams.current < 0) {
                return;
            }
            dirstreams.streams[dirstreams.current]->id = 
                process_int(value->u.object.values[x].value, depth);
        } else if (strcmp(value->u.object.values[x].name, "name") == 0 
                && depth == 1) {
            strcpy(dirstreams.streams[dirstreams.current]->name, 
                    process_string(value->u.object.values[x].value, depth));
        } else if (strcmp(value->u.object.values[x].name, "description") == 0 
                && depth == 1) {
            ptr = process_string(value->u.object.values[x].value, depth);
            if (ptr) {
                strcpy(dirstreams.streams[dirstreams.current]->description, ptr);
            }
        } else if (strcmp(value->u.object.values[x].name, "stream") == 0
                && depth == 2) {
            //TODO: process array of streams
            ptr = process_string(value->u.object.values[x].value, depth);
            if (ptr) {
                strcpy(dirstreams.streams[dirstreams.current]->stream, ptr);
            }
        } else {
